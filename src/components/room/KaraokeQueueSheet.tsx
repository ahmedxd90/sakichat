...
export default function KaraokeQueueSheet({ roomId, isOwner, isAdmin, myProfile, members, onClose }: Props) {
  const songs = useQuery(api.karaoke.getSongs, { roomId });
  const queue = useQuery(api.karaoke.getQueue, { roomId });
  const addToQueue = useMutation(api.karaoke.addToQueue);
  const removeFromQueue = useMutation(api.karaoke.removeFromQueue);
  const startNext = useMutation(api.karaoke.startNext);
  const finishCurrent = useMutation(api.karaoke.finishCurrent);
  const clearQueue = useMutation(api.karaoke.clearQueue);
      ...
      <div className="max-h-[88vh] w-full max-w-lg overflow-hidden rounded-[28px] border border-fuchsia-300/25 bg-[#18052c] text-white shadow-[0_0_60px_rgba(168,85,247,.35)]">
        ...
        <div className="max-h-[calc(88vh-70px)] overflow-y-auto p-4">
          <div className="mb-3 rounded-2xl border border-fuchsia-300/20 bg-fuchsia-950/40 p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                {active && (canManage || String(active.singerId) === String(myProfile?.userId)) && <button type="button" disabled={busy === "finish"} onClick={() => run("finish", () => finishCurrent({ roomId, queueId: active._id, skipped: canManage }))} className="rounded-xl bg-white/10 px-3 py-2 text-xs font-bold">إنهاء</button>}
              </div>
            </div>
          ...
          <div className="mb-5">
            ...
            {(queue ?? []).length === 0 ? <div className="rounded-2xl border border-dashed border-white/15 p-4 text-center text-xs text-white/45">لا توجد أغانٍ في الانتظار</div> : <div className="space-y-2">{(queue ?? []).map((entry: any, index: number) => <div key={entry._id} className={`flex items-center gap-3 rounded-2xl border p-2.5 ${entry.status === "singing" ? "border-fuchsia-300/60 bg-fuchsia-500/15" : "border-white/10 bg-white/5"}`}><div className="flex h-7 w-7 items-center justify-center rounded-full bg-fuchsia-500/25 text-xs font-black">{entry.status === "singing" ? "♫" : index + 1}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{entry.track?.name ?? "أغنية محذوفة"}</p><p className="text-[10px] text-white/55">{entry.singer?.name ?? entry.singerName ?? "مغنٍ"}{entry.status === "singing" ? " · يغني الآن" : ""}</p></div>{entry.status === "queued" && (canManage || String(entry.singerId) === String(myProfile?.userId)) && <button type="button" onClick={() => run(`remove-${entry._id}`, () => removeFromQueue({ roomId, queueId: entry._id }))} className="rounded-lg bg-white/10 px-2 py-1 text-[10px]">إزالة</button>}</div>)}</div>}
          </div>
          ...
          <div>
            ...
            {(songs ?? []).length === 0 ? <div className="rounded-2xl border border-dashed border-white/15 p-4 text-center text-xs text-white/45">لا توجد أغانٍ مرفوعة لهذه الغرفة. ارفع مقطعًا من مشغل الموسيقى أولًا.</div> : <div className="space-y-2">{filteredSongs.map((song: any) => { const alreadyQueued = queuedTrackIds.has(String(song._id)); return <div key={song._id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 p-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-400 to-violet-700 text-lg">♫</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{song.name}</p><p className="text-[10px] text-white/45">{song.duration ? `${Math.round(song.duration)} ثانية` : "مقطع صوتي"}</p></div><button type="button" disabled={alreadyQueued || busy === `add-${song._id}`} onClick={() => run(`add-${song._id}`, () => addToQueue({ roomId, trackId: song._id }))} className="rounded-xl bg-fuchsia-500/90 px-3 py-2 text-[11px] font-black disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/40">{alreadyQueued ? "في الانتظار" : "أضف"}</button></div>; })}</div>}
          </div>
        </div>
      </div>
    </div>
