...
const FootballSeat = memo(function FootballSeat({
  ...
  return (
    ...
    >
      ...
      {member && (
        <div className="flex items-center justify-center gap-0.5 mt-0.5" title={`ثروة LV${member.profile?.wealthLevel ?? 0} وكاريزما LV${member.profile?.charismaLevel ?? 0}`}>
          {(member.profile?.wealthLevel ?? 0) > 0 && <LevelIconSvg level={member.profile.wealthLevel} type="wealth" size={15} />}
          {(member.profile?.charismaLevel ?? 0) > 0 && <LevelIconSvg level={member.profile.charismaLevel} type="charisma" size={15} />}
        </div>
      )}
    </button>
  );
...
export default function FootballSeatsGrid({
  ...
  // مطابقة agoraGlobal.ts حرفياً حتى تصل حالة التحدث إلى المقعد الصحيح.
    ...
    member?.profile?.userId
      ? String(
          Math.abs(
            member.profile.userId
              .split("")
              .reduce((acc: number, c: string) => (acc * 31 + c.charCodeAt(0)) % 100000000, 0)
          )
        )
      : "";
