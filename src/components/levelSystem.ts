...
export function getLevelColor(level: number): {
  ...
  const group = Math.floor((Math.max(level, 1) - 1) / 10);
  const tiers = [
  ...
  return tiers[Math.min(group, tiers.length - 1)];
}

// ── مجموعة المستوى والأصل الرسومي ──
export function getLevelTier(level: number): number {
  return Math.min(5, Math.max(1, Math.ceil(Math.max(level, 1) / 10)));
}

export function getLevelTierLabel(level: number): string {
  const tier = getLevelTier(level);
  return `LV${(tier - 1) * 10 + 1}–LV${tier * 10}`;
}

export function getLevelAsset(level: number, type: "wealth" | "charisma"): string {
  const tier = getLevelTier(level);
  const ranges = ["01-10", "11-20", "21-30", "31-40", "41-50"];
  const range = ranges[tier - 1];
  return `/assets/level-icons/${type}-${type === "wealth" ? "lv" : "lv"}${range}.png`;
}

// ── شكل الأيقونة الاحتياطي للأصول القديمة ──
export function getLevelShapeIndex(level: number): number {
  return Math.min(Math.floor((Math.max(level, 1) - 1) / 10), 4);
}
