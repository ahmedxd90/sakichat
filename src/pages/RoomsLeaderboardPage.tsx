...
export default function RoomsLeaderboardPage({ onBack, onRoomSelect }: RoomsLeaderboardPageProps) {
  ...
  const podiumColors = ["#9ca3af", "#06b6d4", "#0891b2"];
  const podiumGlows = ["rgba(156,163,175,0.4)", "rgba(6,182,212,0.7)", "rgba(8,145,178,0.4)"];
  const podiumHeights = ["h-16", "h-24", "h-12"];
  const podiumSizes = ["w-14 h-14", "w-20 h-20", "w-12 h-12"];
  const crowns = ["🥈", "🏆", "🥉"];

  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: "linear-gradient(180deg, #000a15 0%, #001525 40%, #000a15 100%)" }} dir="rtl">
      {/* Floating particles */}
      {particles.map((p) => (
      ...
      <div className="absolute top-0 left-0 right-0 h-48 pointer-events-none" style={{
        background: "radial-gradient(ellipse at 50% 0%, rgba(6,182,212,0.25) 0%, transparent 70%)",
      }} />

      {/* Bottom glow */}
      <div className="absolute bottom-0 left-0 right-0 h-32 pointer-events-none" style={{
        background: "radial-gradient(ellipse at 50% 100%, rgba(8,145,178,0.12) 0%, transparent 70%)",
      }} />
      ...
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between px-4 pt-12 pb-4 border-b" style={{ borderColor: "rgba(6,182,212,0.15)", background: "rgba(0,10,21,0.6)", backdropFilter: "blur(20px)" }}>
        <button onClick={onBack} className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(6,182,212,0.1)", border: "1px solid rgba(6,182,212,0.25)" }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#06b6d4" strokeWidth="2"><path d="M9 18l6-6-6-6" /></svg>
        </button>
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2">
            <span className="text-2xl" style={{ filter: "drop-shadow(0 0 8px rgba(6,182,212,0.8))" }}>🏠</span>
            <h1 className="font-black text-lg" style={{ background: "linear-gradient(135deg, #a5f3fc, #06b6d4, #0891b2)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>ترتيب الغرف</h1>
            <span className="text-2xl" style={{ filter: "drop-shadow(0 0 8px rgba(6,182,212,0.8))" }}>🏠</span>
          </div>
          <p className="text-cyan-600/60 text-[10px] mt-0.5">أنشط الغرف في SAKU</p>
        </div>
        <div className="w-9" />
      </div>
      ...
      <div className="relative z-10 flex gap-2 px-4 py-3">
        {(["daily", "weekly", "monthly"] as Period[]).map((p) => (
          <button key={p} onClick={() => setPeriod(p)}
            ...
            style={period === p ? { background: "linear-gradient(135deg, #06b6d4, #0891b2)", boxShadow: "0 4px 20px rgba(6,182,212,0.5)" } : { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}
          >
                    ...
                    <button key={idx} className="flex flex-col items-center gap-1 flex-1 active:scale-95 transition-transform" onClick={() => entry && onRoomSelect?.(entry.roomId as Id<"rooms">)}>
                      ...
                      <div className="relative">
                        <div className={`${podiumSizes[idx]} rounded-2xl overflow-hidden`} style={{ boxShadow: `0 0 24px ${podiumGlows[idx]}`, border: `2px solid ${podiumColors[idx]}` }}>
                          {entry.coverUrl ? <img src={entry.coverUrl} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #06b6d4, #0891b2)" }}><span className="text-2xl">🏠</span></div>}
                        </div>
                      </div>
                      <div className="text-center px-1">
                        <p className="text-white font-bold truncate text-[11px]" style={{ maxWidth: "68px" }}>{entry.name}</p>
            ...
            {rest.length > 0 && (
              <div className="px-4 mt-2 space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-cyan-600 text-xs font-bold">المراكز 4 - {data.length}</span>
                  <div className="flex-1 h-px" style={{ background: "rgba(6,182,212,0.2)" }} />
                </div>
                {rest.filter(Boolean).map((entry) => entry && (
                  <button key={entry.roomId} onClick={() => onRoomSelect?.(entry.roomId as Id<"rooms">)} className="w-full flex items-center gap-3 rounded-2xl px-3 py-2.5 active:scale-95 transition-transform" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    ...
                    <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0" style={{ border: "1.5px solid rgba(6,182,212,0.2)" }}>
                      {entry.coverUrl ? <img src={entry.coverUrl} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-cyan-600 to-blue-600 flex items-center justify-center"><span className="text-lg">🏠</span></div>}
                    </div>
                    <div className="flex-1 min-w-0 text-right">
                      <span className="text-white text-xs font-bold truncate block">{entry.name}</span>
                      <p className="text-gray-500 text-[10px]">{entry.ownerName} · {entry.memberCount} عضو</p>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <span className="text-xs">🪙</span>
                      <span className="text-cyan-400 text-xs font-bold">{entry.total >= 1000 ? `${(entry.total / 1000).toFixed(1)}k` : entry.total}</span>
                    </div>
                  </button>
                ))}
              </div>
