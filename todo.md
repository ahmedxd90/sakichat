...
## تدقيق الخطأ الجديد — 2026-08-27
- [ ] استعادة ملف `pasted_content_2.txt` وتحليل الرسالة ومسار الخطأ.
- [ ] تحديد الملف أو الحزمة المتسببة في الخطأ الجديد.
- [ ] تطبيق إصلاح آمن وإعادة فحص TypeScript وVite.
- [ ] إعادة بناء Android الموقّع وتسليم النسخة الجديدة بعد التحقق.

## خطأ إكمال معلومات الحساب بعد Google Login — 2026-08-27
- [x] تتبع مسار RegisterPage وقراءة الخطأ الحقيقي من عمليات Supabase.
- [x] إصلاح رفع الصورة وحفظ profile/user المرتبط بالمستخدم المصادق عليه.
- [x] تحسين رسائل النجاح لتعرض معرف Saki المحفوظ دون كشف تفاصيل تقنية.
- [x] تشغيل TypeScript وVite ثم مزامنة Android بعد الإصلاح.

## تدقيق حفظ saki_id بعد Google Login — 2026-08-27
- [x] التحقق من وجود saki_id في الصفوف الحالية ومن وجوده في نتيجة RPC.
- [x] التحقق من قيود uniqueness وRLS وجدول profiles في Supabase.
- [x] جعل إنشاء saki_id وحفظ profile آمناً أمام التعارض وإعادة المحاولة عبر RPC ذرية.
- [x] إعادة فحص TypeScript والبناء وإصدار Android جديد بعد نجاح الاختبار.

## إعادة بناء صفحة الأرستقراطية — 2026-08-27
- [x] فحص الرابط المرجعي واستخراج التصميم (الألوان، الرتب، المزايا).
- [x] مراجعة `AristocracyPage.tsx` الحالية ومخطط `aristocracy_ranks`.
- [x] تنفيذ الواجهة الجديدة باستخدام Tailwind مع التفاعلات والرسوم المتحركة (framer-motion).
- [x] ربط البيانات الحقيقية (الرتبة الحالية، شريط التقدم، المكافآت) بـSupabase.
- [x] التحقق من استقرار الصفحة في نسخة الويب وبناء Android v1.1.4 المحدّث.

## إصلاح إنشاء الغرفة وعمود country — 2026-08-27
- [x] فحص مخطط `rooms` ومسار إنشاء الغرفة وتحديد الحقول المطلوبة.
- [x] إضافة `country` إلى rooms، ومواءمة `max_seats` مع مخطط Supabase، وتطبيق migration الإنتاج.
- [x] إضافة مخزن `room-covers` وسياسات القراءة والرفع للصور المخصصة.
- [x] تشغيل TypeScript وVite build بنجاح وإعادة بناء Android من GitHub Actions بعد دفع الإصلاح.

## إصلاح RLS عند إنشاء الغرفة — 2026-08-27
- [x] فحص سياسات INSERT في `rooms` وتأكيد تفعيل RLS.
- [x] إضافة سياسات SELECT وINSERT وUPDATE وDELETE آمنة لجدول `rooms` في Supabase.
- [x] ربط `owner_id` بـ `auth.uid()` مباشرة في الكود وتحسين رسائل الخطأ.
- [x] إعادة بناء APK/AAB الموقّعين وتوفير روابط التحميل المصلحة.

## إصلاح lastFinishedPK عند تحميل الغرفة — 2026-08-27
- [ ] البحث عن كل استخدامات وتعريفات `lastFinishedPK` في مسار الغرفة وPK.
- [ ] إصلاح المرجع غير المعرّف مع fallback آمن عند غياب بيانات PK.
- [ ] تشغيل TypeScript وVite واختبار تحميل الغرفة ومزامنة Android.
- [ ] بناء APK/AAB جديدين وتسليم روابط التحميل.

## إصلاح rechargeGiftSettings في HomePage — 2026-08-27
- [ ] تحديد مصدر المرجع غير المعرّف داخل HomePage.
- [ ] إضافة إعدادات هدايا شحن آمنة أو fallback حقيقي من Supabase.
- [ ] استبدال مورد الصورة غير المتاح بمورد ثابت مناسب.
- [ ] تشغيل الفحوصات وإعادة بناء APK/AAB بعد التحقق.
